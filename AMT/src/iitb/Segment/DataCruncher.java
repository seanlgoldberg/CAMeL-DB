...
class TestData {
	...
	int[] groupedTokens() {
		/*
		 * if (grp == null) return null; return grp.groupingArray(seq.length);
		 */
		return null;
	}
		...
		try {
			if ((line = rin.readLine()) != null) {
				...
				for (int i = 0; i < len; i++) {
					...
					if (delimit.indexOf(tokStr) == -1
							|| impDelimit.indexOf(tokStr) != -1) {
...
class TestDataWrite {
	...
	TestDataWrite(String outfile, String rawfile, String delimitP,
			String tagDelimitP, String impDelimitP, LabelMap linfo) {
		try {
			labelmap = linfo;
	...
	void writeRecord(int[] tok, int tokLen) {
		try {
			...
			for (int j = 0; j <= tokLen; j++) {
				if ((j < tokLen) && (t == tok[j])) {
					while (ptr < tokArr.length
							&& delimit.indexOf(tokArr[ptr]) != -1
					...
					while (outputBuffer.charAt(revScanPtr) == ' '
							|| outputBuffer.charAt(revScanPtr) == '('
							|| outputBuffer.charAt(revScanPtr) == '{'
							|| outputBuffer.charAt(revScanPtr) == '[') {
					...
					if (foundOpenChar) {
						outputBuffer = outputBuffer
								.substring(0, revScanPtr + 1);
						ptr -= goBackPtr;
					}
					outputBuffer = new String(outputBuffer + tagDelimit
							+ labelmap.revMap(t));
	...
	void writeRecord(int[] tok, int tokLen, int line) {
		try {
...
public class DataCruncher {
	...
	static String[] getTokenList(String text, String delimit, String impDelimit) {
		StringTokenizer textTok = new StringTokenizer(text.toLowerCase(),
				delimit, true);
		int tlen = 0;
	...
	static int readRowVarCol(int numLabels, BufferedReader tin,
		...
		while (true) {
			...
			// System.out.println("WARNING: duplicate tags in training data are not allowed: "
			// + w);
			// }
			t[ptr] = label;
	...
	static int readRowFixedCol(int numLabels, BufferedReader tin,
			String tagDelimit, String delimit, String impDelimit, int[] t,
			String[][] cArray, int labels[], StringTokenizer rawTok)
			throws IOException {
		String line = tin.readLine();
		if (line == null)
			return 0;
		StringTokenizer firstSplit = new StringTokenizer(line.toLowerCase(),
				tagDelimit, true);
		int ptr = 0;
	...
	static int[] readHeaderInfo(int numLabels, BufferedReader tin,
			...
			while ((rawLine = rin.readLine()) != null) {
				...
				if (fixedColFormat) {
					ptr = readRowFixedCol(numLabels, tin, tagDelimit, delimit,
							impDelimit, t, cArray, labels, rawTok);
				} else {
	...
	public static void readRaw(Vector data, String file, String delimit,
			String impDelimit) {
		try {
