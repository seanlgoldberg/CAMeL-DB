...
public class CRF implements Serializable {
    ...
    public void apply(DataSequence dataSeq,int pos,int label) {
        if (viterbi==null)
            viterbi = getViterbi(1);
        if (params.debugLvl > 1) 
            //Util.printDbg("CRF: Applying on " + dataSeq);
    ...
    public void apply(DataSequence dataSeq,int pos,int label, int labelLeft, int labelRight) {
        if (viterbi==null)
            viterbi = getViterbi(1);
        if (params.debugLvl > 1) 
            //Util.printDbg("CRF: Applying on " + dataSeq);
    ...
    public void apply(DataSequence dataSeq,int pos, int pos2, int label, int label2, int flag) {
        if (viterbi==null)
            viterbi = getViterbi(1);
        if (params.debugLvl > 1) 
            //Util.printDbg("CRF: Applying on " + dataSeq);
