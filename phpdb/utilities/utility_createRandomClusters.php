...
$tokenClassifierMapping = array();
	...
	while ($row = mysql_fetch_assoc($result)) {
		...
		if ($lastCitation != $citationId) {
			if ($lastCitation != 0) {
				$randomIndex = random(0, $frameCount);
				$highestEntropy = $frames[$randomIndex]['highestEntropy'];
				$highestEntropyTokenIndex = $frames[$randomIndex]['tokenIndex'];
				$highestEntropyStart = $frames[$randomIndex]['tokenStart'];
				$highestEntropyGoldStandard = $frames[$randomIndex]['goldStandard'];
				$highestEntropyTokenId = $frames[$randomIndex]['tokenId'];
				$frames = array();
				$frameCount = 0;
				...
				if (isset($targetArray['count'])) {
					...
					doQuery("INSERT INTO citation_clusters (cc_cluster_id, cc_citation_id, cc_clustering_id, cc_dataset_id) VALUES ($nextClusterIndex, $lastCitation, $clusteringId, $dataset)");	
				...
				doQuery("UPDATE citations SET citation_highest_entropy_token=$highestEntropyTokenId WHERE citation_id=$lastCitation AND citation_dataset=$dataset");
		...
		if ($tokenString != ',') {
			$frames[$frameCount] = array();
			$frames[$frameCount]['highestEntropy'] = $entropy;
			$frames[$frameCount]['tokenIndex'] = $tokenIndex;
			$frames[$frameCount]['entityIndex'] = $entityIndex;
			$frames[$frameCount]['start'] = $tokenStart;
			$frames[$frameCount]['goldStandard'] = $goldStandard;
			$frames[$frameCount]['tokenId'] = $tokenId;
